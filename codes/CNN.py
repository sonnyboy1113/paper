...
x = Conv1D(filters = 128, kernel_size = 1)(inputs)
x = MaxPooling1D(pool_size =5)(x)
out = Flatten()(x)
output = Dense(1)(out)
model = Model(inputs=inputs, outputs=output)
model.compile(optimizer='adam',loss='mse')
model.fit(train_batch_dataset,epochs=200,validation_data=test_batch_dataset)
...
if not os.path.exists(results_directory):
    ...
    predict.to_csv(results_directory + f"CNN-y_predict.csv", header=["y_predict"])

#反归一化处理
labels = scaler.inverse_transform(test_labels.reshape(-1, 1))
preds = scaler.inverse_transform(test_preds.reshape(-1, 1))
