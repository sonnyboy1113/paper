...
checkpoint = ModelCheckpoint(
    results_directory + 'best_model.h5',
    monitor='val_loss',
    save_best_only=True,
    verbose=1
)
...
plt.tight_layout()
plt.savefig(results_directory + 'training_history.png', dpi=300, bbox_inches='tight')
plt.show(block=False)

# 保存训练历史
history_df = pd.DataFrame(history.history)
history_df.to_csv(results_directory + 'training_history.csv', index=False)
...
predict_original.to_csv(results_directory + "GRU-y_predict_original.csv", header=["y_predict"])
...
plt.tight_layout()
plt.savefig(results_directory + 'train_test_predictions.png', dpi=300, bbox_inches='tight')
plt.show(block=True)
...
print('\n归一化器已保存到:', results_directory + 'scalers.pkl')
print('最佳模型已保存到:', results_directory + 'best_model.h5')
print('训练历史已保存到:', results_directory + 'training_history.csv')
print('训练曲线图已保存到:', results_directory + 'training_history.png')
print('预测对比图已保存到:', results_directory + 'train_test_predictions.png')