...
gru_model = Sequential([
    GRU(128, input_shape=(5, X_train.shape[1]), return_sequences=True),
    Dropout(0.2),
...
gru_model.fit(train_batch_dataset, epochs=200, validation_data=test_batch_dataset,
              callbacks=[early_stop], verbose=0)

print("开始训练LSTM模型...")
# LSTM 模型
lstm_model = Sequential([
    LSTM(128, input_shape=(5, X_train.shape[1]), return_sequences=True),
    Dropout(0.2),
...
lstm_model.fit(train_batch_dataset, epochs=200, validation_data=test_batch_dataset,
              callbacks=[early_stop], verbose=0)
...
# 计算基础模型的性能
print("\n基础模型性能：")
print("GRU - R²:", r2_score(test_labels, gru_preds))
print("LSTM - R²:", r2_score(test_labels, lstm_preds))
...
predict.to_csv(results_directory + "Stacked-y_predict_normalized.csv", header=["y_predict"])

# ========== 关键修改：使用正确的y_scaler进行反归一化 ==========
...
# 导出原始尺度的预测值
predict_original = pd.DataFrame(final_preds_original)
predict_original.to_csv(results_directory + "Stacked-y_predict_original.csv", header=["y_predict"])
...
plt.title("Stacked Model (LSTM + GRU + XGBoost)", fontsize=14)
