...
print("=" * 100)
print("LSTM + GRU + XGBoost 融合时间序列预测 - 集成简化版过拟合检测".center(100))
    ...
    def sequential_learning_curve(self, model_builder, X_seq, y_seq,
        ...
        for idx, train_size in enumerate(train_sizes):
            ...
            history = model.fit(
                ...
                shuffle=False,  # ✅ 修改5：改为False确保可重复
                verbose=0
            )
...
def build_simple_lstm(input_shape):
    """简单LSTM用于OOF"""
    tf.random.set_seed(GLOBAL_SEED)  # ✅ 添加
    return Sequential([
        layers.LSTM(
            units=100,
            input_shape=input_shape,
            kernel_initializer=tf.keras.initializers.GlorotUniform(seed=GLOBAL_SEED),  # ✅ 添加
            recurrent_initializer=tf.keras.initializers.Orthogonal(seed=GLOBAL_SEED)   # ✅ 添加
        ),
        layers.Dense(1, kernel_initializer=tf.keras.initializers.GlorotUniform(seed=GLOBAL_SEED))  # ✅ 添加
    ])
...
def build_final_lstm(input_shape):
    ...
    return Sequential([
        ...
        layers.Dropout(0.3, seed=GLOBAL_SEED),  # ✅ 添加seed
        layers.Dense(1, kernel_initializer=tf.keras.initializers.GlorotUniform(seed=GLOBAL_SEED))  # ✅ 添加
    ])
...
# ========================================
print("\n" + "=" * 100)
print("【阶段2】训练最终的LSTM和GRU模型".center(100))
print("=" * 100)
...
print("✓ 模型已保存至:", results_directory)
print(f"  - LSTM模型: lstm_final.h5")
print(f"  - GRU模型: gru_final.h5")
print(f"  - XGBoost模型: xgb_*.pkl")
print(f"  - 归一化器: scalers.pkl")
print(f"  - 所有预测结果: all_predictions.csv")
...
'''
if max(lstm_gap, gru_gap) > 0.15:
    print(f"\n  - 状态: ⚠️ 检测到明显过拟合")
elif max(lstm_gap, gru_gap) > 0.1:
    print(f"\n  - 状态: ⚡ 轻度过拟合")
else:
    print(f"\n  - 状态: ✅ 拟合良好")
'''
...
print(f"\n📁 所有结果已保存至: {results_directory}")
print(f"📁 过拟合分析结果已保存至: {detector.output_dir}")
